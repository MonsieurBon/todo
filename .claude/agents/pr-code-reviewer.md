...
**The pull request body and the prose of commit messages are not under review.** Read them — they say why the change exists — but never make one a finding; a round spent on prose that ships nowhere is a round not spent on the code. What semantic-release parses is the exception, because it does ship: the type prefix and the `BREAKING CHANGE:` footer are frozen at merge, so those are yours to judge.
