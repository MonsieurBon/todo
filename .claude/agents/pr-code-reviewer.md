...
**The prose of the pull request body and the commit messages is not under review.** Read it — it says why the change exists, and that often decides whether the code is right — but never make it a finding. Not that the body is stale, not that it omits a decision — with one exception: a claim about what was verified by hand cannot be checked against the diff at all, so where the code it names has moved since, ask whether it still holds. Prose that ships nowhere is re-read on every round, so a finding against it costs a round that belongs on the code. And where the body and the diff disagree, the diff is what is true: judge the diff, and still say nothing else about the body.

**What semantic-release parses is not prose**, so the ban does not reach it: the type prefix, the `BREAKING CHANGE:` footer, a revert's `This reverts commit <sha>.` line, and the subject of a releasing commit, which lands verbatim in `CHANGELOG.md`. These are shipped artifacts, frozen at merge and not correctable on the next push. Judge each against the commit it sits on, among the commits the branch will merge as after its final rebase — not the `fixup!` commits, which are squashed away and ship nothing. One window stays open behind you: a rebase that collapses the branch to a single commit writes a new message for it, after the last round and unreviewed. So where the branch carries more than one surviving commit, say what a collapsed message would still have to carry — the prefix the whole needs, and any `BREAKING CHANGE:` footer, which has to survive the retyping verbatim.

A prefix is wrong in both directions: a behaviour change under a prefix that releases nothing, and a releasing prefix on a change that ships no behaviour. `BREAKING-CHANGE:` with a hyphen is the trap worth knowing — the angular preset this repo runs does not recognise it, and the same words in a subject line are just words, so either silently ships the wrong version. The footer rides no prefix: `chore:` carrying it is still a major.
...
**Release** — CLAUDE.md's Releasing table is the authority on what ships and what does not; read it rather than recalling it. See *What to review* for which parts of a commit message are yours to judge and which commit each is judged against.
