...
The exception is whatever semantic-release consumes — the prefix, the footer, a revert's `This reverts commit <sha>.` line — which is not prose but a shipped artifact, frozen at merge and not correctable afterwards. Judge each against the squashed whole the branch merges as, never against the intermediate commits, which are squashed away and ship nothing:
...
- the **`BREAKING CHANGE:` footer**, which decides the major and is the easiest of these to get wrong. It lives in the commit body, so it is the one piece of a body that is reviewable. `BREAKING-CHANGE:` with a hyphen is not recognised by the angular preset this repo runs, and the same words in the subject line are just words — both ship the wrong version silently;
