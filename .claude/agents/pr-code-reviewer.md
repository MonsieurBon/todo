...
**Release** — CLAUDE.md's Releasing table is the authority on what ships and what does not; read it rather than recalling it, and note the `BREAKING CHANGE:` footer ships a major whatever prefix it sits under. A behaviour change under a prefix that ships nothing is invisible to the release. See *What to review* for which parts of a commit message are yours to judge and which commit each is judged against.
