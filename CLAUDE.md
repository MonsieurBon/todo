...
that led there — the measurements, the run ids, the account of what was tried. Keep it short, and
do not rewrite it every round; it is not a running log of the review. Where evidence settles a
contested point, cite the number, not the working. The one exception is a claim about what was
verified by hand: it covers only the code it was written against, so re-state it where the code it
names has moved — it is the one line the reviewer cannot check against the diff the way it can
check a decision. The same brevity holds for an answer to a review: a line or two per point saying
what was done, or why it wasn't.
